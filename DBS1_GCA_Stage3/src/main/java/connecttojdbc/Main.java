...
public class Main {
    ...
    private void executeQuery5() throws SQLException {
        ...
        try (ResultSet rs = DBCommand.executeQuery(conn, query)) {
            if (!rs.isBeforeFirst()) {
                System.out.println("Still no results found. Possible issues:");
                System.out.println("1. Check if release_year is stored as text (debug shows years as integers)");
                System.out.println("2. Verify manufacturer names exactly match 'Sony' (no hidden characters)");
            } else {
    ...
    private void executeQuery10() throws SQLException {
        try {
            ...
            // Modified query with more flexible criteria
            String query = "SELECT studio_name, number_of_games, " +
                    ...
                    "WHERE number_of_games >= 1 AND latest_release >= date('2020-01-01') " +
                    "ORDER BY number_of_games DESC, latest_release DESC";

            System.out.println("\nExecuting Query 10: Modified Active Studio Statistics\n" + query);
            try (ResultSet rs = DBCommand.executeQuery(conn, query)) {
                if (!rs.isBeforeFirst()) {
                    System.out.println("Still no results. Try removing date filter completely.");
                    String noDateQuery = query.replace("AND latest_release >= date('2020-01-01')", "");
                    try (ResultSet rs2 = DBCommand.executeQuery(conn, noDateQuery)) {
                        DBOutputFormatter.showGenericQueryResult("ALL STUDIOS WITH GAMES", rs2);
                    }
                } else {
